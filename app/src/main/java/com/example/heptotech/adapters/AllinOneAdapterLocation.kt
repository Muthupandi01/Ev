...
class AllinOneAdapterLocation(
    private var brandList: List<FilterLocation>,
    private val context: Context,
    private val onSelectionChanged: (Int, List<FilterLocation>) -> Unit // Callback to update selection count and selected items
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val selectedItems = mutableSetOf<FilterLocation>() // Track selected items
    ...
    fun getSelectedItems(): List<FilterLocation> = selectedItems.toList() // To retrieve selected items

    private fun loadSelectedItems(context: Context): List<FilterLocation> {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val selectedNames = sharedPref.getStringSet("SELECTED_LOCATION", emptySet()) ?: emptySet()

        // Retrieve the selected items based on the names (or IDs)
        val selectedItems = mutableListOf<FilterLocation>()
        for (name in selectedNames) {
            selectedItems.add(brandList.find { it.name == name } ?: FilterLocation(name = name)) // Modify as needed
        }
    ...
    private fun saveSelectedItems(context: Context, selectedItems: List<FilterLocation>) {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val editor = sharedPref.edit()

        // Convert selected items to a set of names or IDs
        val selectedNames = selectedItems.map { it.name }.toSet()
        editor.putStringSet("SELECTED_LOCATION", selectedNames)
