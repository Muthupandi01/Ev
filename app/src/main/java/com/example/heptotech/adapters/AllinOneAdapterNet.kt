...
class AllinOneAdapterNet(
    private var brandList: List<FilterNet>,
    private val context: Context,
    private val onSelectionChanged: (Int, List<FilterNet>) -> Unit // Callback to update selection count and selected items
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val selectedItems = mutableSetOf<FilterNet>() // Track selected items
    ...
    fun getSelectedItems(): List<FilterNet> = selectedItems.toList() // To retrieve selected items

    private fun loadSelectedItems(context: Context): List<FilterNet> {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val selectedNames = sharedPref.getStringSet("SELECTED_NETWORK", emptySet()) ?: emptySet()

        // Retrieve the selected items based on the names (or IDs)
        val selectedItems = mutableListOf<FilterNet>()
        for (name in selectedNames) {
            selectedItems.add(brandList.find { it.name == name } ?: FilterNet(name = name)) // Modify as needed
        }
    ...
    private fun saveSelectedItems(context: Context, selectedItems: List<FilterNet>) {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val editor = sharedPref.edit()

        // Convert selected items to a set of names or IDs
        val selectedNames = selectedItems.map { it.name }.toSet()
        editor.putStringSet("SELECTED_NETWORK", selectedNames)
