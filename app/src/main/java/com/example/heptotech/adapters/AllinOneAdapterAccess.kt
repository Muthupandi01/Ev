...
class AllinOneAdapterAccess(
    private var brandList: List<FilterAccess>,
    private val context: Context,
    private val onSelectionChanged: (Int, List<FilterAccess>) -> Unit // Callback to update selection count and selected items
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val selectedItems = mutableSetOf<FilterAccess>() // Track selected items
    ...
    fun getSelectedItems(): List<FilterAccess> = selectedItems.toList() // To retrieve selected items

    private fun loadSelectedItems(context: Context): List<FilterAccess> {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val selectedNames = sharedPref.getStringSet("SELECTED_ACCESS", emptySet()) ?: emptySet()

        // Retrieve the selected items based on the names (or IDs)
        val selectedItems = mutableListOf<FilterAccess>()
        for (name in selectedNames) {
            selectedItems.add(brandList.find { it.name == name } ?: FilterAccess(name = name)) // Modify as needed
        }
    ...
    private fun saveSelectedItems(context: Context, selectedItems: List<FilterAccess>) {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val editor = sharedPref.edit()

        // Convert selected items to a set of names or IDs
        val selectedNames = selectedItems.map { it.name }.toSet()
        editor.putStringSet("SELECTED_ACCESS", selectedNames)
