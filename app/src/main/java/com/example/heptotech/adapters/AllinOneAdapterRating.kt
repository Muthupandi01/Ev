...
class AllinOneAdapterRating(
    private var brandList: List<Filterrating>,
    private val context: Context,
    private val onSelectionChanged: (Int, List<Filterrating>) -> Unit // Callback to update selection count and selected items
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val selectedItems = mutableSetOf<Filterrating>() // Track selected items
    ...
    fun getSelectedItems(): List<Filterrating> = selectedItems.toList() // To retrieve selected items

    private fun loadSelectedItems(context: Context): List<Filterrating> {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val selectedNames = sharedPref.getStringSet("SELECTED_RATING", emptySet()) ?: emptySet()

        // Retrieve the selected items based on the names (or IDs)
        val selectedItems = mutableListOf<Filterrating>()
        for (name in selectedNames) {
            selectedItems.add(brandList.find { it.name == name } ?: Filterrating(name = name)) // Modify as needed
        }
    ...
    private fun saveSelectedItems(context: Context, selectedItems: List<Filterrating>) {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val editor = sharedPref.edit()

        // Convert selected items to a set of names or IDs
        val selectedNames = selectedItems.map { it.name }.toSet()
        editor.putStringSet("SELECTED_RATING", selectedNames)
