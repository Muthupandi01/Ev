...
class AllinOneAdapterMultipleDevice(
    private var brandList: List<FilterMultidevice>,
    private val context: Context,
    private val onSelectionChanged: (Int, List<FilterMultidevice>) -> Unit // Callback to update selection count and selected items
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val selectedItems = mutableSetOf<FilterMultidevice>() // Track selected items
    ...
    fun getSelectedItems(): List<FilterMultidevice> = selectedItems.toList() // To retrieve selected items

    private fun loadSelectedItems(context: Context): List<FilterMultidevice> {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val selectedNames = sharedPref.getStringSet("SELECTED_MULTIPLE", emptySet()) ?: emptySet()

        // Retrieve the selected items based on the names (or IDs)
        val selectedItems = mutableListOf<FilterMultidevice>()
        for (name in selectedNames) {
            selectedItems.add(brandList.find { it.name == name } ?: FilterMultidevice(name = name)) // Modify as needed
        }
    ...
    inner class BrandViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(brand: FilterMultidevice) {
    ...
    private fun saveSelectedItems(context: Context, selectedItems: List<FilterMultidevice>) {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val editor = sharedPref.edit()

        // Convert selected items to a set of names or IDs
        val selectedNames = selectedItems.map { it.name }.toSet()
        editor.putStringSet("SELECTED_MULTIPLE", selectedNames)
