...
class AllinOneAdapterChargeStation(
    private var brandList: List<FilterChargeStation>,
    private val context: Context,
    private val onSelectionChanged: (Int, List<FilterChargeStation>) -> Unit // Callback to update selection count and selected items
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val selectedItems = mutableSetOf<FilterChargeStation>() // Track selected items
    ...
    fun getSelectedItems(): List<FilterChargeStation> = selectedItems.toList() // To retrieve selected items

    private fun loadSelectedItems(context: Context): List<FilterChargeStation> {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val selectedNames = sharedPref.getStringSet("SELECTED_CHARGESTATION", emptySet()) ?: emptySet()

        // Retrieve the selected items based on the names (or IDs)
        val selectedItems = mutableListOf<FilterChargeStation>()
        for (name in selectedNames) {
            selectedItems.add(brandList.find { it.name == name } ?: FilterChargeStation(name = name)) // Modify as needed
        }
    ...
    inner class BrandViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(brand: FilterChargeStation) {
    ...
    private fun saveSelectedItems(context: Context, selectedItems: List<FilterChargeStation>) {
        val sharedPref = context.getSharedPreferences("USER_SELECTIONS", Context.MODE_PRIVATE)
        val editor = sharedPref.edit()

        // Convert selected items to a set of names or IDs
        val selectedNames = selectedItems.map { it.name }.toSet()
        editor.putStringSet("SELECTED_CHARGESTATION", selectedNames)
